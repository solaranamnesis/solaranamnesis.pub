function renderBooks() {
  ...
  e.innerHTML = "";
    ...
    fetch("books-ta.json").then((t => t.json())).then((t => {
    ...
    loadingIndicator.style.display = "none";
            ...
            function n(t) {
                e.innerHTML = "", t.sort(((t, e) => t.year.match(/\d+/)[0] > e.year.match(/\d+/)[0] ? 1 : e.year.match(/\d+/)[0] > t.year.match(/\d+/)[0] ? -1 : 0)).forEach((t => {
                    ...
                    n.className = "box", n.innerHTML = `\n\t\t\t\t\t\t<strong>\n\t\t\t\t\t\t\t<a href="https://cdn.solaranamnesis.com/library-test/examples/book_shelf-ta.html#${t.id}">${t.title}</a>\n\t\t\t\t\t\t</strong> — ${t.author} (${t.year}) \t\t\n\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t<b>மொழிகள்:</b> ${t.languages}\n\t\t\t\t\t\t\t</small>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t\t<b>பாடங்கள்:</b> ${t.subjects}\n\t\t\t\t\t\t\t\t</small>\n            `, e.appendChild(n)
                }))
            }
            ...
            function l() {
                ...
                n(t.filter((t => {
                    const n = "மொழியைத் தேர்ந்தெடுக்கவும்" === e || t.languages.includes(e),
                        s = "ஆண்டைத் தேர்ந்தெடுக்கவும்" === a || t.year.includes(a),
                        o = "தலைப்பைத் தேர்ந்தெடுக்கவும்" === l || t.subjects.includes(l),
                        h = "தொகுப்பைத் தேர்ந்தெடுக்கவும்" === q || t.collections.includes(q),
                        r = "ஆசிரியரைத் தேர்ந்தெடுக்கவும்" === c || t.author.includes(c);
                    return n && s && o && r && h
                })))
            }
