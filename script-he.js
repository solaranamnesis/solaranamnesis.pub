function renderBooks() {
  ...
  e.innerHTML = "";
    fetch("books-he.json").then((t => t.json())).then((t => {
    if (t && t.length > 0) {
    loadingIndicator.style.display = "none";
        ...
        function l() {
            ...
            n(t.filter((t => {
                const n = "בחר שפה" === e || t.languages.includes(e),
                    s = "בחר שנה" === a || t.year.includes(a),
                    o = "בחר נושא" === l || t.subjects.includes(l),
                    h = "בחר אוסף" === q || t.collections.includes(q),
                    r = "בחר מחבר" === c || t.author.includes(c);
                return n && s && o && r && h
            })))
        }
