function renderBooks() {
  ...
  e.innerHTML = "";
    fetch("books-pt.json").then((t => t.json())).then((t => {
     if (t && t.length > 0) {
    loadingIndicator.style.display = "none";
        ...
        function l() {
            ...
            n(t.filter((t => {
                const n = "Selecionar Idioma" === e || t.languages.includes(e),
                    s = "Selecionar Ano" === a || t.year.includes(a),
                    o = "Selecionar Assunto" === l || t.subjects.includes(l),
                    h = "Selecionar Coleção" === q || t.collections.includes(q),
                    r = "Selecionar Autor" === c || t.author.includes(c);
                return n && s && o && r && h
            })))
        }
