    ...
    .then((t) => {
      if (t && t.length > 0) {
          ...
          ((e.innerHTML = ""),
            t
              ...
              .forEach((t) => {
                ...
                ((n.className = "box"),
                  (n.innerHTML = `\n\t\t\t\t\t\t<strong>\n\t\t\t\t\t\t\t<a href="https://cdn.solaranamnesis.com/library-test/examples/book_shelf-gu.html#${t.id}">${t.title}</a>\n\t\t\t\t\t\t</strong> — ${t.author} (${t.year}) \t\t\n\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t<b>ભાષાઓ:</b> ${t.languages}\n\t\t\t\t\t\t\t</small>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t\t<b>વિષયો:</b> ${t.subjects}\n\t\t\t\t\t\t\t\t</small>\n            `),
                  e.appendChild(n));
              }));
        }
        ...
        function l() {
          ...
          n(
            t.filter((t) => {
              const n = "ભાષા પસંદ કરો" === e || t.languages.includes(e),
                s = "વર્ષ પસંદ કરો" === a || t.year.includes(a),
                o = "વિષય પસંદ કરો" === l || t.subjects.includes(l),
                h = "સંગ્રહ પસંદ કરો" === q || t.collections.includes(q),
                r = "લેખક પસંદ કરો" === c || t.author.includes(c);
              return n && s && o && r && h;
            }),
          );
        }
