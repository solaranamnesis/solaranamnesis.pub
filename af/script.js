      ...
      if (t && t.length > 0) {
          ...
          ((e.innerHTML = ""),
            t
              ...
              .forEach((t) => {
                ...
                ((n.className = "box"),
                  (n.innerHTML = `\n\t\t\t\t\t\t<strong><em>\n\t\t\t\t\t\t\t<a href="https://cdn.solaranamnesis.com/library-test/examples/book_shelf-af.html#${t.id}">${t.title}</a>\n\t\t\t\t\t\t</em></strong> — ${t.author.split(', ').map(n => t.author_url && t.author_url[n] ? `<a href="${t.author_url[n]}" target="_blank" rel="noopener noreferrer">${n}</a>` : n).join(', ')} (${t.year}) \t\t\n\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t<b>Tale:</b> ${t.languages}\n\t\t\t\t\t\t\t</small>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t\t<b>Onderwerpe:</b> ${t.subjects}\n\t\t\t\t\t\t\t\t</small>\n            `),
                  e.appendChild(n));
              }));
        }
        ...
        function l() {
          ...
          n(
            t.filter((t) => {
              const n =
                  'Kies Taal' === e || t.languages.includes(e),
                s = 'Kies Jaar' === a || t.year.includes(a),
                o = 'Kies Onderwerp' === l || t.subjects.includes(l),
                h =
                  'Kies Versameling' === q ||
                  t.collections.includes(q),
                r = 'Kies Outeur' === c || t.author.includes(c);
              return n && s && o && r && h;
            }),
          );
        }
