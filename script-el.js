function renderBooks() {
  ...
  e.innerHTML = "";
    ...
    fetch("books-el.json").then((t => t.json())).then((t => {
    if (t && t.length > 0) {
    loadingIndicator.style.display = "none";
        ...
        function n(t) {
            e.innerHTML = "", t.sort(((t, e) => t.year.match(/\d+/)[0] > e.year.match(/\d+/)[0] ? 1 : e.year.match(/\d+/)[0] > t.year.match(/\d+/)[0] ? -1 : 0)).forEach((t => {
                ...
                n.className = "box", n.innerHTML = `\n\t\t\t\t\t\t<strong>\n\t\t\t\t\t\t\t<a href="https://cdn.solaranamnesis.com/library-test/examples/book_shelf-el.html#${t.id}">${t.title}</a>\n\t\t\t\t\t\t</strong> — ${t.author} (${t.year}) \t\t\n\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t<b>Γλώσσες:</b> ${t.languages}\n\t\t\t\t\t\t\t</small>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t\t<b>Θεματικές:</b> ${t.subjects}\n\t\t\t\t\t\t\t\t</small>\n            `, e.appendChild(n)
            }))
        }
        ...
        function l() {
            ...
            n(t.filter((t => {
                const n = "Επιλέξτε Γλώσσα" === e || t.languages.includes(e),
                    s = "Επιλέξτε Έτος" === a || t.year.includes(a),
                    o = "Επιλέξτε Θέμα" === l || t.subjects.includes(l),
                    h = "Επιλέξτε Συλλογή" === q || t.collections.includes(q),
                    r = "Επιλέξτε Συγγραφέα" === c || t.author.includes(c);
                return n && s && o && r && h
            })))
        }
