...
**Solar Anamnesis Publishing** is a multilingual digital library of public-domain texts. The site serves books in 68 languages, each with localized content typeset in LaTeX and distributed as PDFs via IPFS.

**Live Site:** https://www.solaranamnesis.pub/

**Technical Stack:**
- Static HTML/CSS/JavaScript (vanilla JS; jQuery used only in `book_shelf.html` for the flipbook viewer)
- Python 3.6+ for build scripts (`generate_books.py`)
- Deployed via HTTPS on static CDN (`https://www.solaranamnesis.pub/`)
- `book_shelf.html` files (root + all language editions) are **also** deployed to a separate high-performance CDN at `https://cdn.solaranamnesis.com/library-test/examples/`; the `../css/flipbook.min.css`, `../js/flipbook.min.js`, and `../favicon.ico` paths inside those files resolve correctly relative to that CDN path
- `script.js` book-title links point to the CDN book-shelf URLs intentionally
...
├── links.html              # English links/resources page
├── book_shelf.html         # English flipbook viewer (also deployed to CDN)
├── md-viewer.html          # English Markdown reader
├── shelf_data.html         # English virtual bookshelf (image-based)
├── about.html              # About page
├── timeline.html           # English interactive D3.js timeline viewer
├── ADD_LANGUAGE.md         # Step-by-step guide for adding a new language
├── {lang}/                 # Per-language directories (68 total, e.g., fr/, zh/, ar/)
│   ├── index.html
│   ├── script.js
│   ├── books.json          # Auto-generated by generate_books.py
│   ├── links.html
│   ├── book_shelf.html     # Language-specific flipbook viewer (also deployed to CDN)
│   ├── md-viewer.html      # Language-specific Markdown reader
│   └── timeline.html       # Language-specific interactive D3.js timeline viewer
```
...
**Goal:** Modern, robust, and dependency-free (vanilla JS) — except `book_shelf.html`, which uses jQuery 1.8.3 for the proprietary flipbook viewer.
...
- No jQuery or external framework dependencies (exception: `book_shelf.html` uses jQuery for the flipbook viewer).
...
Each language directory (and the root) contains a `timeline.html` file that renders an interactive D3.js timeline of the book catalogue.

### Overview

- **Technology:** D3.js (loaded from CDN), vanilla JavaScript, inline CSS.
- **Data source:** Reads the sibling `books.json` file at runtime.
- **Purpose:** Visualizes the publication year range of books in the catalogue on a zoomable, scrollable timeline.
- **Year range:** Books span from **1600** to **2022**.
- **Localization:** Each `{lang}/timeline.html` mirrors the root `timeline.html` with translated UI strings (title, description, axis labels, tooltip text, etc.).

### Key Metadata Fields

`timeline.html` files include SEO meta tags that reference the year range. When updating the start year across all files use a bulk `sed` replacement:

```bash
# Replace start year across all timeline.html files
find . -name "timeline.html" -exec sed -i 's/OLD_YEAR/NEW_YEAR/g' {} \;
```

### Structure Notes

- The D3.js timeline script is embedded inline in the `<body>`.
- The year range string (e.g., `"books from 1600 to 2022"`) appears in:
  - The `<meta name="description">` tag.
  - The Open Graph `<meta property="og:description">` tag.
  - The Twitter Card `<meta name="twitter:description">` tag.
  - A JavaScript string literal inside the inline D3 script.
- When the book catalogue's actual year span changes (earliest or latest publication year), update **all** `timeline.html` files simultaneously using the bulk replacement command above.

### Adding a New Language

When adding a new language (see `ADD_LANGUAGE.md`), copy the closest existing `{lang}/timeline.html`, translate the UI strings, and verify the year range matches the current catalogue.
...
See **`ADD_LANGUAGE.md`** for the full step-by-step guide, including automation scripts for bulk-updating existing language directories.

Summary:

1. Add `{lang}` block to `translations.json`.
2. Run `python3 generate_books.py --languages {lang}` to create `{lang}/books.json`.
3. Create `{lang}/index.html`, `script.js`, `links.html`, `book_shelf.html`, `md-viewer.html`, `timeline.html` (copy from closest language and translate).
4. Add `hreflang` link and language picker entry to root `index.html` and every existing `{lang}/index.html`.
5. Add `<url>` block to `sitemap.xml` and entry to `README.md`.
