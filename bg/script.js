    ...
    .then((t) => {
      if (t && t.length > 0) {
        ...
        function l() {
          ...
          n(
            t.filter((t) => {
              const n = "Изберете Език" === e || t.languages.includes(e),
                s = "Изберете Година" === a || t.year.includes(a),
                o = "Изберете Тема" === l || t.subjects.includes(l),
                h = "Изберете Колекция" === q || t.collections.includes(q),
                r = "Изберете Автор" === c || t.author.includes(c);
              return n && s && o && r && h;
            }),
          );
        }
