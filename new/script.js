        ...
        function renderBookList(books) {
          ...
          books
            ...
            .forEach((book) => {
              ...
              li.className = "box";
              li.innerHTML = `\n\t\t\t\t\t\t<strong><em>\n\t\t\t\t\t\t\t<a href="https://cdn.solaranamnesis.com/library-test/examples/book_shelf-new.html#${book.id}">${escapeHtml(book.title)}</a>\n\t\t\t\t\t\t</em></strong> — ${book.author
                .split(", ")
                .map((n) =>
                ...
                .join(
                  ", ",
                )} (${escapeHtml(book.year)}) \t\t\n\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t<b>भाषाहरू:</b> ${escapeHtml(book.languages)}\n\t\t\t\t\t\t\t</small>\n\t\t\t\t\t\t\t<br>\n\t\t\t\t\t\t\t\t<small>\n\t\t\t\t\t\t\t\t\t<b>विषयहरू:</b> ${escapeHtml(book.subjects)}\n\t\t\t\t\t\t\t\t</small>\n            `;
