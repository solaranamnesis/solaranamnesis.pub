function renderBooks() {
  ...
  e.innerHTML = "";
    fetch("books-ru.json").then((t => t.json())).then((t => {
    if (t && t.length > 0) {
    loadingIndicator.style.display = "none";
        ...
        function l() {
            ...
            n(t.filter((t => {
                const n = "Выберите Язык" === e || t.languages.includes(e),
                    s = "Выберите Год" === a || t.year.includes(a),
                    o = "Выберите Тему" === l || t.subjects.includes(l),
                    h = "Выберите Коллекцию" === q || t.collections.includes(q),
                    r = "Выберите Автора" === c || t.author.includes(c);
                return n && s && o && r && h
            })))
        }
